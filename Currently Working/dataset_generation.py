        ...
        try:
            ...
            for fdi in FDIS:
                if fdi in dentition_data_dict['data']:
                    ...
                    # Fill with small random noise if tooth is missing
                    noise = np.random.normal(loc=0.0, scale=0.05, size=(self.tooth_npoints, 3)).astype(np.float32)
            ...
            # Avoid division by zero
            mean = dentition_arr.mean()
            std = dentition_arr.std()
            if std < 1e-6:
                print(f"[NaN Debug] Very small std detected for patient {patient_id}: std={std}, mean={mean}")
                std = 1.0
            dentition_arr_norm = (np.array(dentition_arr) - mean) / std
