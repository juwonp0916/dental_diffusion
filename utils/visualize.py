...
def export_dentition_npy(id, dir, points, separate=False):
...
def export_to_pc_batch(keys, dir, pcs, colors=None):
    ...
    for i, xyz in enumerate(pcs):
        ...
        # pcwrite(os.path.join(dir, 'sample_{}_{}.ply'.format(keys[i],str(i))), xyz, color)

def export_to_pc_individual(path, pc):
    visualize_pointcloud(points = pc, out_file=path+'.png')
    np.save(path+'.npy', pc)
